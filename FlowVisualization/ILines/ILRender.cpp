...
namespace ILines
{
	...
	struct CleanParam {
		int *indicator;
		void *dirt;
		CleanParam(int *indicator, void *dirt) :indicator(indicator), dirt(dirt) {}
	};
	DWORD WINAPI clean_deamon(LPVOID param) {

		CleanParam* p = (CleanParam *)param;
		while (*(p->indicator)!= 0) {
			Sleep(10);
		}
		delete p->indicator;
	                           ...
	                           const float *homVertices) const
	{
		...
		else
		{
			...
			auto res = ILUtilities::zsort(first, vertCount, lineCount, homVertices);
			indices = res.first;
			...
			delete[] indices;
			//if ((*ILUtilities::task_counter) == 0) {
			//	//printf("GC at main thread %ld\n",ILUtilities::task_counter);
			//	delete ILUtilities::task_counter;
			//	ILUtilities::task_counter = 0;
			//	delete res.second;
			//	delete[] indices;

			//	delete[] ILUtilities::dps_gc;
			//	ILUtilities::dps_gc = 0;
			//}
			//else {
			//	CreateThread(0, 0, clean_deamon, new CleanParam(res.second, indices), 0, 0);
			//}
		}
	}
